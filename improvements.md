...
- **`-sheets` (shipped CLI).** Sequential same-named Excel sheet pairs. Full usage contract: [REQUIREMENTS.md §12.1](REQUIREMENTS.md). Short form below. `--a-sheet` / `--b-sheet` remain the single-pair path (not replaced). When last-accept `u` ships, a successful next-sheet `S` is one last-accept unit (restore the previous sheet pair + snapshots + place). Until then, undoing a sheet advance is out of scope.
