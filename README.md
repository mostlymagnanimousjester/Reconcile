...
**Happy path:** launch lands on the **column roster** (row 1 is the largest remaining pending column) → `Enter` into that pile → knock it down with `a` / `A` / pair `y` → **next lever** → edit sources in another tool, save, `r` to refresh. Repeat until **pending = 0**. Regex `/`, sentinel `=`, and Equal / All-matched tabs are real and behind glass. A-only keys, B-only keys, and extras are remaining work from the **overview modal** (`i`) or next lever — they are not column-roster rows.

Home is the **column roster** of pending comparable columns, sorted by pending, then top-pair %, then name. Columns whose shared rows are all equal (or already accepted) are hidden. `Enter` drills in; `Esc` goes back to the parent screen (roster `Esc` stays). `i` opens the overview modal (counts + unmatched/extras); `Esc` closes it. `a` on the roster accepts the focused **column**; grain (pair / cell / one key / extra) is accepted on the matching detail screen. `A` accepts a whole column or all unmatched on a side (from the A-only/B-only grid); `r` re-reads the live files; `q` quits; `?` help.
...
On the roster, `/` drafts comparable columns whose **names** match a Python regex. `=` drafts comparable columns where every **pending** value on one chosen side is exactly a sentinel string (raw text; no trim, no regex, no expression; empty `""` is legal). Both fill a column draft (all `[ON]`); the banner/footer say **y ACCEPT selected**; `Space` select/deselect (`[ON]`/`[off]`); `Esc` cancels. Insights cannot accept. Compare stays exact raw text.
