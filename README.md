...
- **Same-named sequence:** `-sheets` / `--sheets` (`data{1-4,7}` → `data1`…`data4`, `data7`; or a plain list `Jan,Feb`). Do **not** also pass `--a-sheet` / `--b-sheet`. Each expanded name must exist in both workbooks. Only the first pair loads; `S` advances when no draft is open. If that sheet still has pending columns, unmatched keys, or mismatched columns, `S` asks you to leave them unaccepted (`y` or `S` again; `Esc` stays) and does not snapshot them. The last accepted exact A/B strings carry forward so `.` on the next sheet’s pair list can draft that pair. See [REQUIREMENTS.md §12.1](REQUIREMENTS.md).
