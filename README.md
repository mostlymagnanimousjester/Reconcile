...
The footer is status, not a cheat-sheet: counts (**pending columns**, **unmatched keys**, **mismatched columns**) on the left; page / sheet / `? help` on the right; refresh delta on its own line (`pending columns 12→4 · unmatched keys 3→3 · mismatched columns 1→0 · 2 returned Status`). Pair list adds `. repeat · u undo`. Draft recipes live on the **banner**. Full bindings live in the `?` modal, grouped by screen. It does not sum cells + rows + header names into one “cells” figure.
